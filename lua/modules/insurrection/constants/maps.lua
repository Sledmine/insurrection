local stock = {
    {
        name = "beavercreek",
        title = "Battle Creek",
        description = "Splash Splash, Bang Bang\n(2-8 players)",
        author = "Bungie"
    },
    {
        name = "sidewinder",
        title = "Sidewinder",
        description = "Red Blood, White Snow\n(4-16 players, supports vehicles)",
        author = "Bungie"
    },
    {
        name = "damnation",
        title = "Damnation",
        description = "Covenant Hydro-Processing Center\n(4-8 players)",
        author = "Bungie"
    },
    {
        name = "ratrace",
        title = "Rat Race",
        description = "Up the Ramps, Down the Tubes\n(2-6 players)",
        author = "Bungie"
    },
    {
        name = "prisoner",
        title = "Prisoner",
        description = "Get on Top\n(2-8 players)",
        author = "Bungie"
    },
    {
        name = "hangemhigh",
        title = "Hang'Em High",
        description = "Tombstones for Everybody\n(4-16 players)",
        author = "Bungie"
    },
    {
        name = "chillout",
        title = "Chill Out",
        description = "Dude, you really need to...\n(2-8 players)",
        author = "Bungie"
    },
    {
        name = "derelict",
        title = "Derelict",
        description = "Deep-Space Anomaly #0198\n(4-8 players)",
        author = "Bungie"
    },
    {
        name = "boardingaction",
        title = "Boarding Action",
        description = "Ship-to-Ship Combat\n(4-16 players)",
        author = "Bungie"
    },
    {
        name = "bloodgulch",
        title = "Blood Gulch",
        description = "The Quick and the Dead\n(4-16 players, supports vehicles)",
        author = "Bungie"
    },
    {
        name = "wizard",
        title = "Wizard",
        description = "You spin my head right round, right round\n(2-8 players)",
        author = "Bungie"
    },
    {
        name = "putput",
        title = "Chiron TL34",
        description = "Spartan Clone Training Complex\n(2-16 players)",
        author = "Bungie"
    },
    {
        name = "longest",
        title = "Longest",
        description = "A long walk down a short hall...\n(2-8 players)",
        author = "Bungie"
    },
    {
        name = "icefields",
        title = "Ice Fields",
        description = "Slipping and Sliding\n(4-16 players, supports vehicles)",
        author = "Gearbox"
    },
    {
        name = "deathisland",
        title = "Death Island",
        description = "Sand, Surf and Spent Shells\n(4-16 players, supports vehicles)",
        author = "Gearbox"
    },
    {
        name = "dangercanyon",
        title = "Danger Canyon",
        description = "Don't Look Down Unless You Fall\n(4-16 players, supports vehicles)",
        author = "Gearbox"
    },
    {
        name = "infinity",
        title = "Infinity",
        description = "I Imagined it Would Be Bigger\n(4-16 players, supports vehicles)",
        author = "Gearbox"
    },
    {
        name = "timberland",
        title = "Timberland",
        description = "An Enemy Behind Every Tree\n(4-16 players, supports vehicles)",
        author = "Gearbox"
    },
    {
        name = "gephyrophobia",
        title = "Gephyrophobia",
        description = "Scary, huh?\n(2-12 players, supports vehicles)",
        author = "Gearbox"
    }
}

local coop = {
    {
        name = "a10_coop_evolved",
        description = "Escape intact as Covenant forces board \nyour ship.\n(2-4 players)",
        author = "Insurrection Team"
    },
    {
        name = "a30_coop_evolved",
        description = "Seek out surviving Marines and help them\nfight the Covenant.\n(2-4 players)",
        author = "Insurrection Team"
    },
    {
        name = "a50_coop_evolved",
        description = "Board a Covenant ship in an attempt to\nrescue Captain Keyes.\n(2-4 players)",
        author = "Insurrection Team"
    },
    {
        name = "b30_coop_evolved",
        description = "Search for the map room that will lead\nyou to the secrets of Halo.\n(2-4 players)",
        author = "Insurrection Team"
    },
    {
        name = "b40_coop_evolved",
        description = "Defend the Control Room against wave\nafter wave of Covenant troops.\n(2-4 players)",
        author = "Insurrection Team"
    },
    {
        name = "c10_coop_evolved",
        description = "Creep through a swamp to meet the only\nenemy the Covenant fear.\n(2-4 players)",
        author = "Insurrection Team"
    },
    {
        name = "c20_coop_evolved",
        description = "Fight your way through an ancient\nsecurity facility in search of the Index.\n(2-4 players)",
        author = "Insurrection Team"
    },
    {
        name = "c40_coop_evolved",
        description = "Reactivate the weapon at the heart of\nHalo...and learn the truth.\n(2-4 players)",
        author = "Insurrection Team"
    },
    {
        name = "d20_coop_evolved",
        description = "Stage a one-cyborg assault on a Covenant\nship and bring back the Captain.\n(2-4 players)",
        author = "Insurrection Team"
    },
    {
        name = "d40_coop_evolved",
        description = "Destroy Halo before Halo destroys all life in\nthe galaxy.\n(2-4 players)",
        author = "Insurrection Team"
    }
}

local helljumper = {
    {
        name = "aqueduct",
        title = "Aqueduct",
        description = "The resting place of countless warriors from an\never fleeting distant past.\n(2-8 players)",
        author = "Keymind Studios"
    },
        {
        name = "bleed_it_out",
        title = "Bleed It Out",
        description = "Yeah, here we go for a hundred time...\n(4-16 players, supports vehicles)",
        author = "Keymind Studios"
    },
        {
        name = "impasse",
        title = "Impasse",
        description = "These telescopes once were used for looking at\na message from the stars. Now, these silos\ncontain our prepared response.\n(4-16 players, supports vehicles)",
        author = "Keymind Studios"
    },
        {
        name = "last_voyage",
        title = "Last Voyage",
        description = "Though you leave this world, you will come on\nthe Great Journey, when the time comes...\n(2-8 players)",
        author = "Keymind Studios"
    },
        {
        name = "treason",
        title = "Treason",
        description = "The Office of Naval Inteligence leaves no\nloose knots behind...\n(4-16 players)",
        author = "Keymind Studios"
    },
}

return table.extend(stock, coop, helljumper)