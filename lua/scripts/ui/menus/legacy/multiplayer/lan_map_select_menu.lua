...
local widgetPath = container("lan_map_select_menu", {
    {
        header("lan_map_select", "CREATE LEGACY GAME",
        ...
        options("lan_maps_list", "vertical", {
            {
                button("scroll_map_list_up", nil,
                       {legacy = true, func = "single_prev_cl_item_activated"}),
                layout()
            },
            {
                button("map_1", nil,
            ...
            },
            {
                options("map_list_actions", "horizontal", {
                    {
                        button("select_map_ok", "OK", {func = "emit_custom_activation_event"}),
                        pos.back.x - 186,
                        pos.back.y
                    },
                    {button("back", "BACK"), pos.back.x, pos.back.y}
                }, nil, {useItems = true})
            }
