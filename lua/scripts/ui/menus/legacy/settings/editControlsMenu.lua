...
local containerPath =
    [[insurrection\ui\menus\settings_menu\controls_setup\controls_settings_screen.ui_widget_definition]]
...
for buttonIndex = 2, 9 do
    local buttonPath = widget.get(optionsPath, ("child_widgets[%s].widget_tag"):format(buttonIndex))
    local buttonInstance = glue.deepcopy(largeButton("left_justify"))
    buttonInstance.child_widgets = {nil, {horizontal_offset = 70}, {horizontal_offset = 85}}
    widget.edit(buttonPath, buttonInstance)
    ...
    local valuePath = widget.get(buttonPath, "child_widgets[1].widget_tag")
    widget.edit(valuePath, labelButton("center_justify"))

    local valuePath = widget.get(buttonPath, "child_widgets[2].widget_tag")
    widget.edit(valuePath, labelButton("right_justify"))
end

-- Edit buttons bar
local buttonBarPath = widget.get(optionsPath, "child_widgets[10].widget_tag")
widget.edit(buttonBarPath, {
    bounds = "0 0 24 856",
    child_widgets = {
        {horizontal_offset = 40},
        {horizontal_offset = 226},
        {horizontal_offset = 444},
        {horizontal_offset = 630}
    }
})
for buttonIndex = 0, 3 do
